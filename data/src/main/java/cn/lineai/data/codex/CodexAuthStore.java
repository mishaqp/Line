...
/**
 * Encrypted-at-rest storage for the Codex OAuth credential and non-secret
 * identity metadata. OAuth access and refresh tokens never go into the model
 * database or model configuration JSON.
 */
public final class CodexAuthStore {
    private static final String PREFS_NAME = "linecode_codex_auth";
    private static final String TOKENS_KEY = "tokens";
    private static final String IDENTITY_KEY = "identity";
    private static final String KEY_ALIAS = "linecode_codex_auth_v1";
    private static final String SEPARATOR = ":";

    private final Context context;

    public CodexAuthStore(Context context) {
    ...
    public void saveIdentity(String accountId, String planType, String email) throws Exception {
        JSONObject identity = new JSONObject();
        identity.put("account_id", accountId == null ? "" : accountId);
        identity.put("plan_type", planType == null ? "" : planType);
        identity.put("email", email == null ? "" : email);
        writeEncrypted(IDENTITY_KEY, identity.toString());
    }

    public String getAccountId() {
        return identityValue("account_id");
    }
    ...
    private JSONObject readJson(String key) {
        ...
        try {
            ...
            // A stale/invalidated Keystore entry must not crash the app during
            // startup or a background token refresh.
            context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
                    .edit()
                    .remove(key)
                    .apply();
